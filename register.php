...
if ($stmt = $con->prepare('SELECT id, password FROM accounts WHERE username = ?')) {
	...
	if ($stmt->num_rows > 0) {
        ...
        if ($stmt = $con->prepare('INSERT INTO accounts (username, password, email, fname, sname) VALUES (?, ?, ?, ?, ?)')) {
		// Nie chcemy ujawniać haseł w naszej bazie danych, więc haszujemy hasło i używamy password_verify, gdy użytkownik się zaloguje.
            ...
            $stmt->bind_param('sssss', $_POST['username'], $password, $_POST['email'], $_POST['fname'], $_POST['sname']);
            $stmt->execute();                       
